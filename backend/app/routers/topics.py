...
from app.db.models.topic import Topic as TopicModel # Pre prístup k TopicStatus enum z modelu
...
from app.db.enums import AchievementCriteriaType, TopicStatus # Importuj enum priamo
...
def create_topic_for_subject_route(
    ...
    created_topic_orm = crud_topic.create_topic(
        db=db, 
        topic_in=topic_payload, # Predpokladáme, že CRUD funkcia očakáva tento parameter
...
def update_topic_route(
    ...
    # Ulož si pôvodný status pre porovnanie
    original_status = original_topic.status

    updated_topic_orm = crud_topic.update_topic(
        db, 
        topic_id=topic_id, 
        topic_update=topic_update_payload,
        owner_id=current_user.id
    )
    if updated_topic_orm is None: # Z redundantnosti, už by sa to malo stať vyššie
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Topic not found or not authorized to update")
...
def delete_topic_route(
    ...
    # Po zmazaní témy prepočítaj všetky relevantné achievementy
    check_and_grant_achievements(db, current_user) # Skontroluj všetky, keďže to môže ovplyvniť viacero kritérií
