...
export default function StudyMaterialUploadForm({ subjectId, onUploadSuccess, token }: StudyMaterialUploadFormProps) {
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (!title.trim()) {
        const fileNameWithoutExtension = event.target.files[0].name.split('.').slice(0, -1).join('.');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-1 border rounded-lg bg-card">
      <div className="space-y-1.5">
        <Label htmlFor="material-file-input">Súbor <span className="text-destructive">*</span></Label>
        ...
        {selectedFile && (
          <p className="text-xs text-muted-foreground flex items-center mt-1">
            <FileText className="h-3 w-3 mr-1" /> Vybrané: {selectedFile.name} ({(selectedFile.size / 1024).toFixed(1)} KB)
          </p>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Input id="material-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Napr. Prednáška 1: Úvod do..." />
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="material-type">Typ materiálu (voliteľné)</Label>
        <Select value={materialType} onValueChange={(value) => setMaterialType(value as MaterialTypeEnum)}>
          <SelectTrigger id="material-type"><SelectValue placeholder="Vyberte typ" /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1.5">
        ...
        <Textarea id="material-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Stručný popis obsahu materiálu..." rows={2}/>
      </div>
